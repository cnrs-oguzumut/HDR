...
\begin{frame}{Scientific Context: Nonlinearity \& Instability}
    \begin{columns}[T, onlytextwidth]
        % Left Column: Challenges and Phenomena
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{Unifying Perspective}
                \vspace{0.2cm}
                Treating these diverse phenomena as \textit{\color{AcademicNavy} displacive instabilities} driven by the competition between \textbf{elastic energy} and \textbf{lattice symmetry}.
            \end{block}
            
            \vspace{0.5cm}
            \centering
            \begin{tikzpicture}
                \draw[AcademicNavy, ultra thick, -{Stealth[scale=1.2]}] (0,0) -- (0,-1.5) node[midway, right] {\small \textit{Research Goal}};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
